...
<div class="w-full max-w-7xl mx-auto" x-data="{ 
    ...
    <style> <!-- Kullanıcı fotoğrafının seçilmemesi için -->
        .user-photo {
            ...
            @foreach($users as $user)
                <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-5" :class="view === 'list' ? 'flex items-center space-x-4' : ''">
                    ...
                    <div class="flex-1" :class="view === 'grid' ? 'text-center mt-4' : ''">
                        ...
                        <div class="mt-4" :class="view === 'grid' ? 'text-center' : 'text-right'">
                            <a href="{{ route('admin.users.edit', $user->id) }}" class="inline-block px-4 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition">Düzenle</a>
                        </div>
                    </div>
                </div>
            @endforeach
