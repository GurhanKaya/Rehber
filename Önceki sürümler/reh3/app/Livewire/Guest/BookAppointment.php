...
class BookAppointment extends Component
{
    ...
    public function findNextAvailableSlot($showMessage = true)
    {
        ...
        for ($i = 0; $i < 5; $i++) {
            ...
            if ($slotDays->contains($checkDate->dayOfWeek)) {
                ...
                if ($hasAvailableSlot) {
                    ...
                    $this->loadSlots();
                    if ($showMessage) {
                        $this->successMessage = 'En yakın uygun günü bulduk: ' . $checkDate->format('d.m.Y');
                    }
                    return;
                }
            }
        ...
        if ($showMessage) {
            $this->errorMessage = 'Önümüzdeki 5 gün içinde uygun randevu saati bulunamadı.';
        }
    }
    ...
    public function book()
    {
        ...
        if (!$this->selectedInterval || !$this->name) {
            $this->errorMessage = 'Lütfen bir saat seçin ve adınızı girin.';
            return;
        }
        ...
        $exists = Appointment::where('user_id', $this->user->id)
            ...
            ->exists();
        if ($exists) {
            $this->errorMessage = 'Bu saat için zaten randevu alınmış.';
        ...
        $this->successMessage = 'Randevunuz başarıyla alındı!';
        $this->loadSlots();
