    ...
    <div class="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-2xl shadow-xl">
        <div class="px-8 py-6 border-b border-gray-200 dark:border-zinc-700">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ __('app.update_profile') }}</h1>
        ...
        <form wire:submit.prevent="update" class="p-8 space-y-8">
                ...
                <div class="flex items-start gap-6">
                    ...
                    <div class="shrink-0">
                        ...
                        @elseif($currentPhotoPath)
                            <div class="relative">
                                <img class="h-32 w-32 object-cover rounded-full border-4 border-white dark:border-zinc-700 shadow-lg" 
                                     src="{{ Storage::url($currentPhotoPath) }}" alt="Mevcut profil fotoğrafı">
                        ...
                        @else
                            <div class="h-32 w-32 rounded-full bg-gradient-to-br from-blue-400 to-purple-500 flex items-center justify-center border-4 border-white dark:border-zinc-700 shadow-lg">
                                <span class="text-3xl font-bold text-white">{{ auth()->user()->initials() }}</span>
                            </div>
                        @endif
                    </div>
            ...
            <div class="bg-gray-50 dark:bg-zinc-800 rounded-xl p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" wire:model.defer="name" placeholder="{{ __('app.your_name') }}"
                    ...
                    <div>
                        ...
                        <input type="text" wire:model.defer="surname" placeholder="{{ __('app.your_surname') }}"
                    ...
                    <div>
                        ...
                        <input type="email" wire:model.defer="email" placeholder="{{ __('app.example_email') }}"
                    ...
                    <div>
                        ...
                        <input type="tel" wire:model.defer="phone" placeholder="{{ __('app.example_phone') }}"
            ...
            <div class="bg-gray-50 dark:bg-zinc-800 rounded-xl p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select wire:model.defer="title" class="w-full px-4 py-3 border border-gray-300 dark:border-zinc-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-zinc-700 dark:text-white @error('title') border-red-500 @enderror">
                            <option value="">{{ __('app.all_titles') }}</option>
                            @foreach($titles as $titleOption)
                                <option value="{{ $titleOption }}" {{ $title == $titleOption ? 'selected' : '' }}>{{ $titleOption }}</option>
                            @endforeach
                        </select>
                        @error('title') 
                            <span class="text-red-600 dark:text-red-400 text-sm mt-1 block">{{ $message }}</span> 
                    ...
                    <div>
                        ...
                        <select wire:model.defer="department" class="w-full px-4 py-3 border border-gray-300 dark:border-zinc-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-zinc-700 dark:text-white @error('department') border-red-500 @enderror">
                            <option value="">{{ __('app.all_departments') }}</option>
                            @foreach($departments as $deptOption)
                                <option value="{{ $deptOption }}" {{ $department == $deptOption ? 'selected' : '' }}>{{ $deptOption }}</option>
                            @endforeach
                        </select>
                        @error('department') 
                            <span class="text-red-600 dark:text-red-400 text-sm mt-1 block">{{ $message }}</span> 
            ...
            <div class="bg-gray-50 dark:bg-zinc-800 rounded-xl p-6">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">{{ __('app.language_settings') }}</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-1">
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            {{ __('app.interface_language') }}
                        </label>
                        <select wire:model.defer="locale" class="w-full px-4 py-3 border border-gray-300 dark:border-zinc-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-zinc-700 dark:text-white @error('locale') border-red-500 @enderror">
