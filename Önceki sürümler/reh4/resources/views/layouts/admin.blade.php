...
<head>
    ...
    <title>{{ __('app.admin_panel') }}</title>
...
<body class="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 min-h-screen flex flex-col">
    <nav class="bg-gray-900 shadow-md p-4 text-white" role="navigation" aria-label="{{ __('app.main_navigation') }}">
        <div class="container mx-auto flex justify-between items-center">
            <a href="{{ route('admin.home') }}" class="font-bold text-xl hover:text-gray-300 transition focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-900 rounded" aria-label="{{ __('app.admin_panel_home_aria') }}">{{ __('app.admin_panel') }}</a>
            
            <!-- Desktop Menu -->
            <div class="hidden lg:flex items-center space-x-2 xl:space-x-4">
                <!-- Language Switcher -->
                <div class="flex items-center space-x-2">
                    <a href="{{ route('language.change', 'tr') }}" class="px-3 py-2 text-sm font-medium rounded-md {{ app()->getLocale() == 'tr' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600' }} transition">
                        TR
                    </a>
                    <a href="{{ route('language.change', 'en') }}" class="px-3 py-2 text-sm font-medium rounded-md {{ app()->getLocale() == 'en' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600' }} transition">
                        EN
                    </a>
                </div>
                ...
                <a href="{{ route('admin.profile.edit') }}" class="flex items-center font-semibold px-4 py-2 rounded-md bg-teal-600 hover:bg-teal-700 transition focus:outline-none focus:ring-2 focus:ring-teal-500 text-sm xl:text-base">
                    <svg class="w-4 h-4 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                    </svg>
                    <span>{{ __('app.my_profile') }}</span>
                </a>
                <x-logout-button class="ml-2" />
            </div>
        ...
        <div id="mobileMenu" class="hidden lg:hidden mt-4 space-y-2 border-t border-gray-700 pt-4" role="menu">
            ...
            <div class="flex items-center justify-center space-x-2 pb-2">
                <a href="{{ route('language.change', 'tr') }}" class="px-4 py-2 text-sm font-medium rounded-md {{ app()->getLocale() == 'tr' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600' }} transition">
                    TR
                </a>
                <a href="{{ route('language.change', 'en') }}" class="px-4 py-2 text-sm font-medium rounded-md {{ app()->getLocale() == 'en' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600' }} transition">
                    EN
                </a>
            </div>
            ...
            <a href="{{ route('admin.profile.edit') }}" class="flex items-center px-4 py-3 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition focus:outline-none focus:ring-2 focus:ring-teal-500" role="menuitem">
                <svg class="w-4 h-4 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                </svg>
                <span>{{ __('app.my_profile') }}</span>
            </a>
            <x-logout-button class="block mt-2 w-full text-left flex items-center justify-center" />
        </div>
    </nav>
